...
{
    C = multiplyMatricesByStrassen(A, B, size, threadsAmount, true);
}
...
std::vector<int> multiplyMatricesByStrassen(const std::vector<int>& A, const std::vector<int>& B, int size, int threadsAmount, bool isNewThreads) {
    if (size > 32) {
        ...
        if (isNewThreads)
        {
            diversificationTasksByThreads(threadsAmount, D, D1, D2, H1, H2, V1, V2, A11, A12, A21, A22, B11, B22, B21, B22, size);
        } else
        {
            std::vector<int> D(newSize); D = multiplyMatricesByStrassen(matricesAddition(A11, A22, size/2), matricesAddition(B11, B22, size/2), size/2, 0, false);
            std::vector<int> D1(newSize); D1 = multiplyMatricesByStrassen(matricesSubtraction(A12, A22, size/2), matricesAddition(B21, B22, size/2), size/2, 0, false);
            std::vector<int> D2(newSize); D2 = multiplyMatricesByStrassen(matricesSubtraction(A21, A11, size/2), matricesAddition(B11, B12, size/2), size/2, 0, false);
            std::vector<int> H1(newSize); H1 = multiplyMatricesByStrassen(matricesAddition(A11, A12, size/2), B22, size/2, 0, false);
            std::vector<int> H2(newSize); H2 = multiplyMatricesByStrassen(matricesAddition(A21, A22, size/2), B11, size/2, 0, false);
            std::vector<int> V1(newSize); V1 = multiplyMatricesByStrassen(A22, matricesSubtraction(B21, B11, size/2), size/2, 0, false);
            std::vector<int> V2(newSize); V2 = multiplyMatricesByStrassen(A11, matricesSubtraction(B12, B22, size/2), size/2, 0, false);
        }
